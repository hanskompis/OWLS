...
\subsection{OWL- ja RDFS -ontologiat}

RDF Schema mahdollistaa yksinkertaisten ontologioiden muodostamisen \cite{RDFS}. RDFS:llä on mahdollista määrittää resurssien joukkoja luokiksi (\textit{class}). Luokalla voi olla aliluokkia(\textit{subclass}) ja luokkien jäsenet ovat ilmentymiä (\textit{instance}). Ilmentymien välisiä suhteita kutsutaan \textit{suhteiksi} ja suhteilla on myös \textit{alisuhteita}. Seuraavassa esimerkissä kuvataan ontologia, joka käsittelee polkupyörien valmistajia. Resurssi \textit{Surly} kuuluu luokkaan (\texttt{rdfs:Class})\textit{Polkupyöräpaja}. Luokkaan kuuluminen ilmaistaan \texttt{rdf:type} -konstruktiolla:

\begin{verbatim}
    <rdfs:Class rdf:ID="Surly" />
...
\begin{verbatim}
    <rdfs:Class rdf:ID="Polkupyorapaja" />
